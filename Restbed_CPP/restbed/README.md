...
------------------
* Build and Install the openssl from Externel Source
```bash
    wget https://www.openssl.org/source/openssl-3.3.1.tar.gz
    tar -C <DestinationDir> -xvzf openssl-3.3.1.tar.gz
    cd openssl-3.3.1
    sudo ./Configure --prefix=/usr/local/ssl --openssldir=/usr/local/ssl '-Wl,-rpath,$(LIBRPATH)'
```
or
...
```bash
    ...
    sudo ./Configure --prefix=/usr/local/ssl --openssldir=/usr/local/ssl '-Wl,-rpath,$(LIBRPATH)'
```
* Build and Install the catch
```bash
    cd RESTFUL_api/Restbed_CPP/restbed/dependency/catch
    mkdir build
    cd build
    cmake ..
    sudo make install
```
* Build and Install the asio
```bash
    cd RESTFUL_api/Restbed_CPP/restbed/dependency/asio/asio
    ./autogen.sh
    ./configure
    sudo make install
```
* Building Restbed with openssl and asio support
```bash
    cd RESTFUL_api/Restbed_CPP/
    mkdir restbed/build
    cd restbed/build
    cmake ..
    make install
...
* Building Restbed without openssl and asio support
```bash
    cd RESTFUL_api/Restbed_CPP/
    mkdir restbed/build
    cd restbed/build
